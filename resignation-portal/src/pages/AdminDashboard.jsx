...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchResignations = async () => {
      try {
        const res = await fetch('http://localhost:3001/api/admin/resignations', {
  ...
  const handleApproval = async (resignationId, action) => {
    try {
      const res = await fetch('http://localhost:3001/api/admin/conclude_resignation', {
        method: 'PUT',
        headers: {
  ...
  useEffect(() => {
    const fetchResponses = async () => {
      const res = await fetch('http://localhost:3001/api/admin/exit_responses', {
        headers: {
    ...
    <div className="p-4">
      ...
      {resignations.map((r) => (
        <div key={r._id} className="border p-4 rounded shadow bg-white mb-4">
          ...
          <p><strong>Employee ID:</strong>  {r.employeeId?.username || 'Employee'}</p>
          <p><strong>LWD:</strong> {r.lwd || 'Not set'}</p>
      ...
      {data.map((record, idx) => (
        <div key={idx} className="border p-4 mb-4 rounded">
          <p className="font-semibold">Employee ID: {record.employeeId._id}</p>
